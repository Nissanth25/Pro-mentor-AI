...
// ── CONFIG ──────────────────────────────────────────────────────
let OPENAI_KEY = localStorage.getItem('pm_openai_key') || '';
const OPENAI_URL = 'https://api.openai.com/v1/chat/completions';

// ── MERMAID ──────────────────────────────────────────────────────
mermaid.initialize({
  startOnLoad: false, theme: 'dark',
  themeVariables: { darkMode: true, background: '#0c0c1a', primaryColor: '#7c3aed', primaryTextColor: '#e8e8f5', lineColor: '#5a5a7a', fontSize: '13px' }
});
...
function loadProjects() {
  ...
  const raw = localStorage.getItem(`pm_projects_${currentUser.uid}`);
  projects = raw ? JSON.parse(raw) : [];
}
function saveProjects() {
  if (!currentUser) return;
  localStorage.setItem(`pm_projects_${currentUser.uid}`, JSON.stringify(projects));
}
function getStats() {
  if (!currentUser) return { projects: 0, docs: 0, chats: 0, features: 0 };
  return JSON.parse(localStorage.getItem(`pm_stats_${currentUser.uid}`) || '{"projects":0,"docs":0,"chats":0,"features":0}');
}
function saveStats(s) {
  if (!currentUser) return;
  localStorage.setItem(`pm_stats_${currentUser.uid}`, JSON.stringify(s));
}
...
async function runAnalysis() {
  ...
  try {
    ...
    showToast('⚠️ API Quota/Key issue. Running in offline high-fidelity mode...', 'error');
...
async function sendChatMessage() {
  ...
  typingEl.innerHTML = `<div class="msg-av">🤖</div><div class="chat-typing"><span></span><span></span><span></span></div>`;
...
function appendChatMsg(role, text) {
  ...
  div.innerHTML = role === 'assistant'
    ? `<div class="msg-av">🤖</div><div class="msg-bubble">${formatChatText(text)}</div>`
...
openaiKeyInput.value = localStorage.getItem('pm_openai_key') || '';
...
['settings-close', 'settings-cancel'].forEach(id => {
  const btn = document.getElementById(id);
  if (btn) btn.addEventListener('click', () => toggleSettingsModal(false));
});
...
function applyTheme(t) {
  ...
  if (!t || t === 'default' || t === 'system') { document.body.removeAttribute('data-theme'); localStorage.setItem('pm_theme', 'system'); return; }
...
if (onboardClose) onboardClose.addEventListener('click', () => { showOnboarding(false); localStorage.setItem(ONBOARD_KEY, '1'); });
if (onboardSkip) onboardSkip.addEventListener('click', () => { showOnboarding(false); localStorage.setItem(ONBOARD_KEY, '1'); });
...
try {
  ...
  else if (!localStorage.getItem(ONBOARD_KEY) && !params.get('autorun')) setTimeout(() => showOnboarding(true), 900);
} catch(e) {}
...
document.getElementById('settings-save').addEventListener('click', () => {
  ...
  localStorage.setItem('pm_openai_key', key);
  OPENAI_KEY = key;
